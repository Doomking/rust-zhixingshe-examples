...
async fn main() -> Result<()> {
    ...
    info!("AI Backend: {}", std::env::var("AI_BASE_URL").unwrap_or_default());
...
async fn handle_connection(mut socket: TcpStream) -> Result<()> {
    ...
    loop {
        ...
        if data.windows(12).any(|w| w == b"lock_screen\n") {
            ...
            // 收到音频流块，打印输出
            info!("[AUDIO] Received {} bytes. Data snippet: {:02x?}", n, &data[..16.min(n)]);
            // TODO: 此处对接唤醒词检测引擎 (Porcupine/Precise)
        }
    }
...
fn trigger_macos_lock() {
    ...
    let private_api_success = unsafe {
        match libloading::Library::new("/System/Library/PrivateFrameworks/login.framework/Versions/Current/login") {
            Ok(lib) => {
                match lib.get::<unsafe extern "C" fn()>(b"SACLockScreenImmediate") {
                    Ok(lock_func) => {
