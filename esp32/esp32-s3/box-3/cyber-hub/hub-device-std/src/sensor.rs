...
pub fn sensor_thread(mut i2c_dev: I2cDriver<'static>) {
    info!("[SENSOR] AHT30 thread started.");

    // Step 1: Initialization
    let mut initialized = false;
    for _i in 1..=3 {
        if i2c_dev.write(AHT30_ADDR, &CMD_INIT, 100).is_ok() {
            thread::sleep(Duration::from_millis(50));
            let mut status = [0u8; 1];
            if i2c_dev.read(AHT30_ADDR, &mut status, 100).is_ok() {
                if (status[0] & 0x08) != 0 {
    ...
    loop {
        ...
        for _ in 1..=3 {
            if i2c_dev.write(AHT30_ADDR, &CMD_MEASURE, 100).is_err() {
            ...
            let mut data = [0u8; 7];
            if i2c_dev.read(AHT30_ADDR, &mut data, 100).is_ok() {
                if (data[0] & 0x80) != 0 {
                    thread::sleep(Duration::from_millis(100));
                    continue;
                }
