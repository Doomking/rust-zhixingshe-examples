...
impl<'a> Es7210<'a> {
    ...
    pub async fn init(&mut self) -> Result<(), ()> {
        ...
        self.write_reg(0x43, 0x1F)?; 
        self.write_reg(0x44, 0x1F)?;
        self.write_reg(0x45, 0x1F)?;
        self.write_reg(0x46, 0x1F)?;
        self.write_reg(0x47, 0x08)?; 
        self.write_reg(0x48, 0x08)?; 
        self.write_reg(0x49, 0x08)?; 
        self.write_reg(0x4A, 0x08)?; 
...
pub async fn audio_record_task(
    ...
    let mut transfer = i2s_rx.read_dma_circular_async(rx_buffer).expect("DMA RX START FAIL");
        ...
        match transfer.pop(dma_chunk).await {
            Ok(n) if n > 0 => {
                ...
                if last_report.elapsed() > Duration::from_secs(2) {
                    info!("[DIAG] {} blocks | SAMPLE: {:02x}{:02x}", received_blocks, dma_chunk[0], dma_chunk[1]);
                    received_blocks = 0;
...
pub async fn dummy_tx_task(
    ...
    let mut transfer = i2s_tx.write_dma_circular_async(tx_buffer).expect("TX DMA FAIL");
