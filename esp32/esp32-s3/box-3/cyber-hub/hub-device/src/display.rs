...
use crate::fonts::{draw_chinese_char, draw_icon};
    ...
    D: DrawTarget<Color = Rgb565> + OriginDimensions,
{
    ...
    // 整体上移避开底部黑边并修正遮挡
    let y1 = 135; // Row 1 (CPU/TEMP)
    let y2 = 180; // Row 2 (RAM/HUM) 保持足够间距
    let label_y_offset = 32;
    let icon_y_offset = 22; // 图标基准深度下放，对齐数值腰部
    ...
    clear_area(
        display,
        Rectangle::new(Point::new(env_offset, y1), Size::new(145, 45)),
    )?;
    draw_icon(
        display,
        "temp_icon",
        Point::new(env_offset + 5, y1 + icon_y_offset),
        env_color,
    )?;
    ...
    clear_area(
        display,
        Rectangle::new(Point::new(env_offset, y2), Size::new(145, 45)),
    )?;
    draw_icon(
        display,
        "hum_icon",
        Point::new(env_offset + 5, y2 + icon_y_offset),
        env_color,
    )?;
    ...
    D: DrawTarget<Color = Rgb565> + OriginDimensions,
{
    ...
    if !desc_raw.is_empty() {
        ...
        let x = (320 - total_w) / 2;
        // 紧缩位置 (v24)
        Text::new(&full_str, Point::new(x, 125), font_style).draw(display)?;
    } else {
        Text::new(
            "WEATHER UPDATING...",
            Point::new(95, 125),
...
fn format_num_spaced<'a>(buf: &'a mut [u8], val: u8) -> &'a str {
    let v = if val > 99 { 99 } else { val };
    buf[0] = b'0' + (v / 10);
    buf[1] = b'0' + (v % 10);
    buf[2] = b' ';
